(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    // the members, it also maintains an array of results.
      var iteratedResultsArray = [];

      _.each(collection, function(item){
        iteratedResultsArray.push(iterator(item));
      })
    return iteratedResultsArray;
  };
  ...
  _.every = function(collection, iterator) {
    ...
    if (iterator===undefined) {
      return _.reduce(collection, function(memo, item){
        ...
        return _.reduce(collection, function(memo, item) {
          return memo && !!iterator(item); }, true);
      }
  };
  ...
  _.extend = function(obj) {
    ...
    _.each(args, function(properties) {
  ...
  _.shuffle = function(array) {
    ...
    var hasItems = shuffleArray.length; 
    var tempValue; 
    var randomIndex;
    while (hasItems) {
      randomIndex = Math.floor(Math.random() * hasItems--);
      tempValue = shuffleArray[hasItems];
  ...
  _.invoke = function(collection, functionOrKey, args) {
    if (typeof functionOrKey === 'function') {
      ...
      return _.map(collection, function(item){
        var appliedMethod = item[functionOrKey].apply(item, args);
  ...
  _.zip = function() {
    var fullArray=[];
    var passedArray = Array.prototype.slice.call(arguments); 
    
    _.each(passedArray, function(itemOuter) {
      fullArray.push([]);
    });

    _.each(passedArray, function(itemOuter) {
      for (var i = 0; i < passedArray.length; i++) {
        fullArray[i].push(itemOuter[i]);
      }
    });
    return fullArray;
  };
  ...
  _.intersection = function() {
    ...
    var numOfArray = arrays.length;
    var values = {};
    var intersected = [];
    _.each(arrays, function(item){
      _.each(item, function(nestValue) {
        if (values[nestValue] === undefined) {
          values[nestValue] = 1;
        } else {
          values[nestValue] += 1;
        }
    ...
    _.each(values, function(prop, key) {
      if (+prop === numOfArray) {
        intersected.push(key);
      }
    })
    return intersected;
  };
  ...
  _.difference = function(array) {
    ...
    _.each(values, function(prop, key) {
      if (+prop === 1 && _.indexOf(array, +key) != -1) {
        diff.push(+key);
      }
    })
    return diff;
